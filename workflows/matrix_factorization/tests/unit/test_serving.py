...
from workflows.matrix_factorization.serving import RecommendRequest
...
@pytest.fixture
def test_app(model_pkl: str, monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    """Configure the serving app with test model path and return it."""

    monkeypatch.setenv("MODEL_PATH", model_pkl)
    monkeypatch.setenv("MODEL_DATA_CAPTURE_PATH", str(tmp_path / "inference.jsonl"))
    monkeypatch.setenv("MODEL_DATA_CAPTURE_ENABLED", "true")
...
@pytest.mark.asyncio
async def test_health_after_model_load(test_app):
...
async def test_recommend_known_user(test_app):
    ...
    async with AsyncClient(transport=ASGITransport(app=test_app), base_url="http://test") as client:
        resp = await client.post(
            "/recommend",
            json=RecommendRequest(user_id=1, top_k=5),
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data[CFG_BATCH_PREDICTION_FIELD_NAMES.USER_ID.value] == 1
        assert len(data[CFG_BATCH_PREDICTION_FIELD_NAMES.RECOMMENDATIONS.value]) == 5
        assert all(
            CFG_PREDICTION_FIELD_NAMES.ITEM_ID.value in r
            and CFG_PREDICTION_FIELD_NAMES.SCORE.value in r
            for r in data[CFG_BATCH_PREDICTION_FIELD_NAMES.RECOMMENDATIONS.value]
        )
        # Scores should be descending
        scores = [
            r[CFG_PREDICTION_FIELD_NAMES.SCORE.value]
            for r in data[CFG_BATCH_PREDICTION_FIELD_NAMES.RECOMMENDATIONS.value]
        ]
...
async def test_recommend_unknown_user(test_app):
    ...
    async with AsyncClient(transport=ASGITransport(app=test_app), base_url="http://test") as client:
        resp = await client.post(
            "/recommend",
            json=RecommendRequest(user_id=9999, top_k=10),
        )
...
async def test_recommend_writes_log(test_app, tmp_path, monkeypatch):
    """POST /recommend should write a log entry to MODEL_DATA_CAPTURE_PATH."""
    import serving.app as app_module

    log_path = tmp_path / "inference.jsonl"
    monkeypatch.setenv("MODEL_DATA_CAPTURE_PATH", str(log_path))
    app_module._model = make_test_model()
    app_module.MODEL_DATA_CAPTURE_PATH = str(log_path)
    app_module.MODEL_DATA_CAPTURE_ENABLED = True
    app_module.MODEL_DATA_CAPTURE_BATCH_SIZE = 10
    app_module._inference_log_buffer.clear()

    async with AsyncClient(transport=ASGITransport(app=test_app), base_url="http://test") as client:
        await client.post(
            "/recommend",
            json=RecommendRequest(user_id=1, top_k=3),
        )

    app_module._flush_inference_logs(force=True)
    ...
    assert len(lines) == 1
    assert lines[0][CFG_RECS_LOG_FIELD_NAMES.USER_ID.value] == 1
    assert lines[0][CFG_RECS_LOG_FIELD_NAMES.TOP_K.value] == 3
