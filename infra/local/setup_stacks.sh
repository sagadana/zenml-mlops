...
ZENML_SERVER_URI_DOCKER_RAW="${ZENML_SERVER_URI_DOCKER:-http://host.docker.internal:${ZENML_SERVER_PORT:-8237}}"
MLFLOW_TRACKING_URI_DOCKER_RAW="${MLFLOW_TRACKING_URI_DOCKER:-http://host.docker.internal:${MLFLOW_TRACKING_PORT:-5000}}"
...
ZENML_SERVER_URI_DOCKER="$(strip_wrapping_quotes "${ZENML_SERVER_URI_DOCKER_RAW}")"
MLFLOW_TRACKING_URI_DOCKER="$(strip_wrapping_quotes "${MLFLOW_TRACKING_URI_DOCKER_RAW}")"
...
if zenml orchestrator describe "${ZENML_LOCAL_ORCHESTRATOR_NAME}" >/dev/null 2>&1; then
  zenml orchestrator update "${ZENML_LOCAL_ORCHESTRATOR_NAME}" \
    --env "ZENML_STORE_URL=${ZENML_SERVER_URI_DOCKER}" \
...
if zenml experiment-tracker describe "${ZENML_LOCAL_EXPERIMENT_TRACKER_NAME}" >/dev/null 2>&1; then
  zenml experiment-tracker update "${ZENML_LOCAL_EXPERIMENT_TRACKER_NAME}" \
    --tracking_uri="${MLFLOW_TRACKING_URI_DOCKER}" \
...
echo "  ✓ Experiment tracker: ${ZENML_LOCAL_EXPERIMENT_TRACKER_NAME} (uri=${MLFLOW_TRACKING_URI_DOCKER})"
